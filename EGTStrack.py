...
class EGTStrack(object):
    ...
    def add_service(self, record_types, *args, **kwargs):
        ...
        if record_types == 1:
            ...
            kwargs['speed'] = kwargs['speed'] * 10
            # скорость и направление
            if kwargs['angle'] < 255:
                dir = int(kwargs['angle']).to_bytes(1, byteorder='little')
                spd = int(kwargs['speed']).to_bytes(2, byteorder='little')
            else:
                spd = bitstring_to_bytes("11" + bin(kwargs['speed'])[2:].rjust(14, '0'), 'little')
                dir = bitstring_to_bytes(bin(kwargs['angle'])[3:], 'little')

            odm = int(120).to_bytes(3, byteorder='little')

            # цифровые датчики 1 byte
            din = b'\x01'

            # 1 byte Источник сообщения "по таймеру при включенном зажигании"
            src = b'\x00'
